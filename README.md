# Hi there, I'm Nishtha 👋

🎓 B.Tech CSE student at JECRC University, Jaipur | Minor in Data Science (AI-ML) from IIT Mandi

💻 Currently focused on **DSA (Python)**, **Core CS subjects (OS, CN, DBMS)**, and **AI/ML**

🚀 I love building real-world projects that solve everyday problems

---

### 🔧 What I'm working on
- Strengthening DSA fundamentals (Striver's A2Z Sheet)
- AI/ML learning path (Apna College Prime)
- Building and improving full-stack mini projects with Python & Flask

---

### 🛠️ Tech Stack
**Languages:** Python, C, C++  
**Web/Backend:** Flask, HTML, CSS  
**Tools & Concepts:** OpenCV, Selenium, SQLite, Git/GitHub  
**Currently Learning:** Machine Learning, Data Structures & Algorithms

---

### 📌 Featured Projects
- 🪪 [Passport Photo Generator](https://github.com/nish-coder04/passport-photo-generator) — AI-powered background removal tool using OpenCV & rembg
- 🛒 [Price Comparison Tool](https://github.com/nish-coder04/Price-comparison) — Quick-commerce price comparator (Blinkit, Zepto, Instamart) using Flask & Selenium
- 🌐 [Portfolio Website](https://github.com/nish-coder04/portfolio_NS) — My personal portfolio site

---

### 🌐 Connect with me
- 🔗 [Portfolio](https://nishtha-ns.vercel.app/)
- 💼 [LinkedIn](https://www.linkedin.com/in/nishtha-s-0b213b318/)


---

⭐ *Always open to learning, collaborating, and building cool things!*
